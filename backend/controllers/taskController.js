...
const updateTask = async (req, res) => {
  try {
    ...
    // ✅ Update assignedTo (must be array)
    ...
    // ✅ Auto-manage status based on todos
    if (task.todoChecklist.length > 0) {
      ...
      // If no todos, use provided status or keep existing
      task.status = req.body.status || task.status || "Pending";
    }
